...
var sendResponse = function (err, req, res, data) {
  if (err) {
    if (err.status == 1) res.status(500);
    else res.status(400);
  } else {
    res.send(data);
  }
...
router.get("/getAll", function (req, res, next) {
  sendResponse(null, req, res, products);
});

router.get("/getByCategory/:categoryId", function (req, res, next) {
  const categoryId = req.params.categoryId;
  sendResponse(
    null,
    req,
    res,
    lodash.filter(products, function (item) {
